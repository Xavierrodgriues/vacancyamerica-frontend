import { Search, Circle, Camera, Edit3, ArrowLeft, Send, Phone, Video, Info, Image, Smile, Mic, X } from "lucide-react";
import { useState, useRef, useEffect } from "react";

// ─── Shared types & data (same as RightSidebar) ────────────────────────────
interface ChatUser {
    id: string;
    name: string;
    username: string;
...
const DUMMY_MESSAGES: Record<string, Message[]> = {
    ...
    "2": [
        ...
        { id: "m2", senderId: "2", text: "No problem at all! They were really impressed", time: "9:05 AM", isMine: false },
...
function MobileChatView({ user, onBack }: { user: ChatUser; onBack: () => void }) {
    const [newMessage, setNewMessage] = useState("");
    const [messages, setMessages] = useState<Message[]>(DUMMY_MESSAGES[user.id] || DEFAULT_MESSAGES);
    ...
    const handleSend = () => {
        ...
        setMessages((prev) => [...prev, {
            ...
            time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }), isMine: true,
        }]);
        setNewMessage("");
    };
        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-4 space-y-3" style={{ scrollbarWidth: "thin" }}>
                <div className="flex flex-col items-center py-4 mb-2">
                    ...
                    <h4 className="text-sm font-bold text-foreground mt-2">{user.name}</h4>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 relative">
                        <input
                            ...
                            className="w-full px-4 py-2.5 bg-muted/50 border border-post-border rounded-full text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-blue-500 transition-all pr-16"
                        />
                    ...
                    {newMessage.trim() && (
                        <button onClick={handleSend} className="p-2.5 bg-blue-600 hover:bg-blue-500 rounded-full transition-colors flex-shrink-0">
...
// ─── Main Mobile Chat Panel (full-screen overlay) ───────────────────────────
export function MobileChatPanel({ onClose }: { onClose: () => void }) {
    ...
    return (
        <div className="fixed inset-0 z-40 bg-background flex flex-col" style={{ paddingBottom: "0" }}>
            {activeChat ? (
                <MobileChatView user={activeChat} onBack={() => setActiveChat(null)} />
            ) : (
                <>
                    {/* Header */}
                    ...
                    <div className="px-4 py-2 border-b border-post-border flex-shrink-0">
                        ...
                        <div className="flex gap-3 overflow-x-auto pb-1">
                            {DUMMY_CHATS.filter((c) => c.online).map((user) => (
                                <button key={user.id} onClick={() => setActiveChat(user)} className="flex flex-col items-center gap-1 group">
